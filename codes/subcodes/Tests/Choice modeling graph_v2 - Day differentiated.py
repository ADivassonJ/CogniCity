...
# ============================================================
# ✅ FIGURA → RESUMEN GLOBAL (CORREGIDA)
# ============================================================
...
for scen in SCENARIOS:
    ...
    scen_df = modal_avg[modal_avg["Scenario"] == scen].copy()

    # =====================================================
    # ✅ CREAR GLOBAL PARA ESTE SCENARIO
    # =====================================================
    scen_global = (
        modal_df[modal_df["Scenario"] == scen]
        .groupby("Scenario")
        .mean(numeric_only=True)
        .reset_index()
    )

    scen_global["citizen_archetype"] = "GLOBAL"

    # =====================================================
    # ✅ UNIR ARQUETIPOS + GLOBAL
    # =====================================================
    scen_df = pd.concat([scen_df, scen_global], ignore_index=True)

    # rename SOLO después de añadir GLOBAL
    scen_df["citizen_archetype"] = scen_df["citizen_archetype"].map(rename_archetypes).fillna(scen_df["citizen_archetype"])
    ...
    # ✅ FIX: construir order desde el índice real del pivot
    order = [x for x in pivot.index if x != "GLOBAL"] + (["GLOBAL"] if "GLOBAL" in pivot.index else [])
    ...
    ax.set_xlabel("Archetypes + Global")
    ax.set_ylim(0, 100)
