...
def plot_heatmap_panel_5(mats: dict, s_order: list[str], title: str, colors: dict, vmin=0.0, vmax=100.0):
    # Gradiente simple: 0% -> blanco, 100% -> s0
    cmap = LinearSegmentedColormap.from_list("custom_heatmap", ["#ffffff", colors["s0"]])
    ...
    for ax, s in zip(axes, s_order):
        ...
        mappable = im
        ax.set_title(proper_names.get(s), fontsize=11)
...
def run_one_case(case_name: str, out_dir: Path, colors):
    ...
    # Curva doméstica (igual para todos los escenarios)
    domestic_curve = total_power_per_hour(demand_by_time_base)

    # Contenedores para curvas
    mobility_curves = {}
    total_curves = {}
    ...
    for s in S_FOLDERS:
        ...
        # Curvas agregadas del sistema
        mob_curve = total_power_per_hour(mob_by_time)
        tot_curve = total_power_per_hour(demand_total)

        mobility_curves[s] = mob_curve
        total_curves[s] = tot_curve
    ...
    # Heatmaps
    title = f""
    fig = plot_heatmap_panel_5(mats=mats, s_order=S_FOLDERS, title=title, colors=colors, vmin=0.0, vmax=100.0)

    # ------------------------------------------------------------
    # Plot comparación de potencia total del sistema
    # ------------------------------------------------------------
    # Encontrar línea(s) que salen del root
    root_children = [n for n, p in parent.items() if p == root]

    root_line_capacities = []
    for ch in root_children:
        ek = edge_key(root, ch)
        if ek in pmax_map:
            root_line_capacities.append(pmax_map[ek])

    if root_line_capacities:
        system_capacity_mw = sum(root_line_capacities)
    else:
        system_capacity_mw = None
...
def main():
    ...
    for case_name in STUDY_CASES:
        ...
        run_one_case(case_name, out_dir, colors)

# ============================================================
# TOTAL POWER CURVES (system level)
# ============================================================

def total_power_per_hour(demand_by_time: dict) -> np.ndarray:
    """
    Suma potencia total del sistema por hora.
    Returns array shape (24,) en MW.
    """
    totals = np.zeros(24)
    for h in range(24):
        totals[h] = sum(demand_by_time.get(h, {}).values())
    return totals


def plot_power_curves(case_name,
                      domestic_curve,
                      total_curves,
                      system_capacity_mw=None, 
                      colors=None):
    ...
    # --------------------------------------------------------
    # 1) Domestic demand como área
    # --------------------------------------------------------
    plt.fill_between(
        hours,
        domestic_curve,
        alpha=0.25,
        color=colors.get("base", "grey"),
    ...
    # --------------------------------------------------------
    # 2) Escenarios totales (líneas discontinuas + marcadores distintos)
    # --------------------------------------------------------
    markers = {
        "s0": "o",
        "s1": "s",
    ...
    for s in S_FOLDERS:
        ...
        plt.plot(
            ...
            markersize=5,
            markevery=1,   # marcador cada 2 horas para claridad
            label=f"{proper_names.get(s)} EVs",
        )
