# ============================================================
# HEATMAP SLOW ACUMULATIVO (aguas arriba) — 7 días — tomar día 3
# Panel comparativo: 5 mapas de calor (s0..s4) EN UNA SOLA FIGURA
# -> UNA ÚNICA barra de color (leyenda degradado) por study_case
#
# ROOT por caso de estudio (según indicas):
#   - Annelinn: Node_43
#   - Aradas: Node_20
...
# ✅ ROOT correcto por caso de estudio
root_map = {
    "Annelinn": "Node_43",
    "Aradas": "Node_20",
    "Kanaleneiland": "Node_67",
...
def find_mobility_excel(base_path: str, s_folder: str, study_case: str) -> str:
    ...
    if len(files) == 0:
        raise FileNotFoundError(f"No se encontró Excel (quantified_24) en: {folder}\nPatrón: {pattern}")
    files = sorted(files, key=lambda f: os.path.getmtime(f), reverse=True)
    if len(files) > 1:
        print(f"[WARN] Múltiples Excels en {folder}. Usando el más reciente:\n  {files[0]}")
    return files[0]

# ===================== 1) Markers por agente y día =====================
def compute_agents_home_markers_by_day(xlsx_path: str, sheet_name=0) -> pd.DataFrame:
    ...
    if missing:
        raise ValueError(f"[{xlsx_path}] Faltan columnas: {missing}. Disponibles: {list(df.columns)}")
...
# ===================== 3) Simulación SLOW local por nodo/hora =====================
...
def build_weighted_graph(nodes: pd.DataFrame, edges: pd.DataFrame):
    ...
    if "node_label" not in nodes.columns:
        if "i" in nodes.columns:
            ...
            raise ValueError("nodes debe tener 'i' o 'node_label'")
...
# ===================== 7) Panel 5 heatmaps (s0..s4) + 1 colorbar =====================
def plot_heatmap_panel_5(mats: dict, s_order: list[str], title: str, vmin: float, vmax: float):
    """
    5 heatmaps (s0..s4) en una fila + 1 colorbar
    La colorbar tiene EXACTAMENTE la misma altura que los heatmaps.
    """
    ...
    one_w = 24 * 0.22
    cbar_w = 0.35  # ancho de la columna de colorbar

    fig = plt.figure(figsize=(one_w * 5 + cbar_w, fig_height))

    # 🔴 GridSpec: 5 heatmaps + 1 colorbar
    gs = fig.add_gridspec(
        nrows=1,
        ncols=6,
    ...
    cax = fig.add_subplot(gs[0, 5])  # eje exclusivo para la colorbar
    ...
    # 🔴 Colorbar con la misma altura que los heatmaps
    cbar = fig.colorbar(mappable, cax=cax)
    cbar.set_label("Potencia acumulada (subárbol) [kW]")
...
def run_one_study_case(study_case: str):
    root = root_map[study_case]  # ✅ ya no asumimos todos iguales
    ...
    # ✅ TODOS los buses alcanzables desde root (incluye los que no cargan)
    ...
    if SHOW_NETWORK_ONCE:
        net_title = f"RED — {study_case} — root {root} (anillo=bus con carga en algún s; flechas=árbol Dijkstra)"
    ...
    panel_title = f"{study_case} — Heatmaps SLOW acumulativos (día 3: {DAY_NAMES_7[DAY_TO_SHOW_INDEX]}) — root {root}"
