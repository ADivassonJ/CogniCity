...
def todolist_family_initialization(pop_building, family_df, activities): # esta funcion necesita tambien relacion de familias y arquetipos
    ...
    todolist_family = todolist_family.sort_values(by=['out', 'in'], ascending=[True, True]).reset_index(drop=True)
...
def todolist_family_creation(df_citizens, pop_building):
    ...
    for family_name, family_df in tqdm(df_citizens_families, desc="Procesando familias"):
        ...
        todolist_family = todolist_family_LEVEL_2(todolist_family, pop_building)
...
def todolist_family_LEVEL_2(todolist_family, pop_building,  decision_time = 60):
    # Sacamos las actividades que requieren asistencia
    dependents = todolist_family[todolist_family['todo_type'] != 0]
    # Logramos las lineas de los helpers que pueden encargarse de ayudar
    helpers = todolist_family[
        (todolist_family['todo_type'] == 0) & 
        (~todolist_family['agent'].isin(dependents['agent'])) &
        (todolist_family['in'] != 0)
    ]
    
    # Producto cartesiano (todas las combinaciones posibles)
    cartesian = helpers.merge(dependents, how='cross')
        
    for idx_car, row_car in cartesian.iterrows():
        # Distances calculation
        cartesian.loc[idx_car, 'time_dist'] = time_dist_calculate(row_car)
        cartesian.loc[idx_car, 'geo_dist']  = geo_dist_calculate(row_car, pop_building)
    ...
    cartesian.sort_values(by='in_x', ascending=True)
        
    cartesian_filtered = cartesian.loc[cartesian.groupby(['agent_x', 'agent_y', 'todo_y', 'osm_id_y'])['score'].idxmin()].sort_values(by='in_y', ascending=True).reset_index(drop=True)
        
    cartesian_grouped = cartesian_filtered.groupby(['agent_y', 'todo_y', 'osm_id_y'])
        
    step = cartesian_filtered['agent_x'].nunique()
    i = 0

    while i < len(cartesian_filtered):
        base_row = cartesian_filtered.iloc[i]
        group_end = i + step

        while (
            group_end < len(cartesian_filtered) and
            all(
                cartesian_filtered.iloc[group_end][col] == base_row[col]
                for col in ['agent_x', 'todo_x', 'osm_id_x']
            )
        ):
            group_end += step
       
        df = cartesian_filtered.iloc[i:group_end]  # Subgrupo del DataFrame

        print(df)
        
        # Número de combinaciones (k)
        k = df['agent_y'].nunique()

        # Lista para guardar combinaciones válidas
        combinaciones_validas = []

        # Generar todas las combinaciones posibles de k filas
        for indices in itertools.combinations(df.index, k):
            rows = df.loc[list(indices)]
            
            # Verificar que todos los 'agent_y' sean distintos
            if rows['agent_y'].nunique() == k:
                combinaciones_validas.append(indices)

        # Crear nuevo DataFrame con información resumida de cada combinación válida
        pares_df = pd.DataFrame([
            {
                **{f'agent_x_{j+1}': df.loc[idx, 'agent_x'] for j, idx in enumerate(indices)},
                **{f'agent_y_{j+1}': df.loc[idx, 'agent_y'] for j, idx in enumerate(indices)},
                'tot_score': sum(df.loc[idx, 'score'] for idx in indices)
            }
            for indices in combinaciones_validas
        ])
    ...
    print('cartesian_grouped')
    input(cartesian_grouped)
            
def time_dist_calculate(row_car):
    return abs(row_car['in_x'] - row_car['in_y'])

def geo_dist_calculate(row_car, pop_building):
    ...
    return haversine(lat_h, lon_h, lat_d, lon_d)
