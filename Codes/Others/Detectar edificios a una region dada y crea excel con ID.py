...
def obtener_edificios_ciudad(ciudad):
    ...
    ID_edificios = edificios_poligonos.index.get_level_values('osmid').tolist()
    
    return edificios_poligonos, ID_edificios
...
def crear_archivo_excel(ID_edificios):
    # Crear un DataFrame de pandas con los datos de los edificios
    df = pd.DataFrame({'ID': ID_edificios})
...
def main():
    ...
    edificios, ID_edificios = obtener_edificios_ciudad(ciudad)
    
    # Crear un archivo Excel con los datos de los edificios
    crear_archivo_excel(ID_edificios)
    ...
    ruta_guardado_mapa = 'mapa_edificios.html'
