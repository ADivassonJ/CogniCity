...
def initialize_population(df_buildings, population, EV_percentage):
    ...
    df_actions, df_citizens, df_vehicles = initialize_dataframes()
    ...
    # Procesar vehículos
    df_vehicles, df_actions = process_vehicles(vehicles_names, df_actions, df_buildings, day, time, df_vehicles)
...
def process_citizens(citizen_names, living_buildings, working_buildings, vehicles_names, df_buildings, day, time, df_citizens, df_actions):
    ...
    for citizen in tqdm(citizen_names, desc="       citizens: ", bar_format="{desc}{percentage:3.4f}%"):
        ...
        vehicle_on_property = random.choice(vehicles_names) if vehicles_names else None

        # Añadir ciudadano
        citizens_data.append({'agent_name': citizen, 'archetype': archetype, 'link_to': None, 
                              'vehicle': vehicle_on_property, 'Living': living_on, 'Working': working_on})

        # Añadir acción
        osmid = random.choice([living_on, working_on])
        service_building = 'Living' if osmid == living_on else 'Working'
        bus = df_buildings.loc[df_buildings['osmid'] == osmid, 'bus'].iloc[0]
        actions_data.append({'osmid': osmid, 'service': service_building, 'day': day, 'time': time, 
...
def process_vehicles(vehicles_names, df_actions, df_buildings, day, time, df_vehicles):
    ...
    for vehicle in tqdm(vehicles_names, desc="       vehicles: ", bar_format="{desc}{percentage:3.4f}%"):
        ...
        # Añadir vehículo
        vehicles_data.append({'agent_name': vehicle, 'archetype': archetype, 'SoC': SoC})
