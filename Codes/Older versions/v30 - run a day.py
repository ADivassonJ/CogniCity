...
# v29   ->  Limpieza general
# v30   ->  heatmaps more options
...
def next_move(df_buildings, df_citizens, data, intention, steps_captured):
    ...
    for idx, row in df_buildings.iterrows():
        if intention in row['services']:
            ...
            print(building_coord, building_dest)
            input()
...
def archetype_probability_movement(archetype, hour):
    ...
    directorio_base = r"C:\Users\asier.divasson\Desktop\V2G-QUESTS\Behaviour Models"
...
def create_heatmap(df_actions, df_buildings, area, directory, use_areas=False):
    # Obtener las coordenadas o geometrías de los edificios
    if use_areas:
        # Convertir la columna 'geometry' a listas de coordenadas
        def parse_geometry(geom):
            try:
                # Si geom es un objeto WKT, convertirlo a un objeto Polygon de shapely
        ...
        building_geoms = dict(zip(df_buildings['osmid'], df_buildings['geometry'].apply(parse_geometry)))
    else:
        building_geoms = dict(zip(df_buildings['osmid'], df_buildings['coord']))
    ...
    for hour in grouped_actions['time'].unique():
        ...
        # Crear una lista de geometrías y sus respectivos conteos para la hora actual
        heatmap_data = []
        for _, row in hour_actions.iterrows():
            osmid = row['osmid']
            count = row['count']
            geom = building_geoms.get(osmid)
            if geom:
                # Invertir latitud y longitud para cada punto en la geometría
                geom = [(lon, lat) for lat, lon in geom]
                heatmap_data.append((geom, count))
        ...
        # Crear el mapa con un fondo más claro
        if use_areas:
            # Pintar áreas (polígonos de edificios)
            center_lat = df_buildings['coord'].apply(lambda x: x[0]).mean()
            center_lon = df_buildings['coord'].apply(lambda x: x[1]).mean()
            m = folium.Map(location=[center_lat, center_lon], zoom_start=12, tiles='cartodbpositron')
            
            # Normalizar los conteos para que el color varíe entre azul (mínimo) y rojo (máximo)
            counts = [count for _, count in heatmap_data]
            min_count, max_count = min(counts), max(counts)
            
            # Función para calcular el color basado en el conteo
            def get_color(count):
                norm_count = (count - min_count) / (max_count - min_count + 1e-5)  # Normalización entre 0 y 1
                r = int(255 * norm_count)   # Rojo va aumentando
                b = int(255 * (1 - norm_count))  # Azul va disminuyendo
                return f'#{r:02x}00{b:02x}'  # Genera el color hexadecimal

            # Dibujar polígonos con los colores adecuados
            for geom, count in heatmap_data:
                if geom:  # Verificar que geom no esté vacío
                    folium.Polygon(
                        locations=geom,
                        fill=True,
                        fill_color=get_color(count),
                        fill_opacity=min(0.2 + count * 0.05, 1.0),
                        stroke=False
                    ).add_to(m)
        else:
            # Pintar puntos centrales con un fondo claro
            heatmap_df = pd.DataFrame(heatmap_data, columns=['lon', 'lat', 'count'])
            m = folium.Map(location=[heatmap_df['lat'].mean(), heatmap_df['lon'].mean()], zoom_start=12, tiles='cartodbpositron')
...
def simulate_day(df_actions, df_citizens, df_buildings, moving_agents, day, steps_captured):
    ...
    for time in range(steps_captured):
        ...
        df_actions, moving_agents = process_agents(df_actions, df_citizens, df_buildings, moving_agents, time, day, steps_captured)
...
def process_agents(df_actions, df_citizens, df_buildings, moving_agents, time, day, steps_captured):
...
if __name__ == "__main__":
    ...
    # Inicialización
    df_buildings, df_actions, df_citizens, df_vehicles, df_moving_agents = initialization(area, population_citizens, EV_percentage, directory, type_services)
    
    # Bucle de simulación
    for day in range(days_captured):
        print('#' * 22, 'day:', day + 1, '#' * 22)                           ####### COSORRO no se puc actualizar un df en un for
        df_actions, df_moving_agents = simulate_day(df_actions, df_citizens, df_buildings, df_moving_agents, day, steps_captured)
    
    create_heatmap(df_actions, df_buildings, area, directory, False)
    
    # Guardar los DataFrames
    df_names = ['df_buildings', 'df_actions', 'df_citizens', 'df_vehicles', 'df_moving_agents']
    df_group = [df_buildings, df_actions, df_citizens, df_vehicles, df_moving_agents]
    save_dataframes(directory, df_group, df_names)
