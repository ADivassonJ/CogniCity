...
def initialize_SoC(population):
    return [0]*population

def initialize_population(services_buildings, population, agents_names, EV_names):
    ...
    SoC = initialize_SoC(population) # Initialize SoC of EV
    i = 0

    for living in services_buildings.get('Living', []):
        for _ in range(residents_per_building):
            link_to = random.choice(EV_names)
            code = f"{link_to},{day},{time},{agents_names[i]},in,citizen,0,{SoC[i]}"
...
def initialization(ciudad, population, directory, type_services, agents_names, EV_names):
    ...
    buildings_info = initialize_population(services_buildings, population, agents_names, EV_names)

    data_for_df = []
    for osmid, agent_data_list in buildings_info.items():
        for agent_data in agent_data_list:
            link_to, day, time, agent_name, in_out, agent_type, archetype, SoC = agent_data.split(',')
            data_for_df.append({
                'osmid': str(osmid),
                'link_to': link_to,
                'day': int(day),
...
def next_move(df_services, data, buildings_df, intention):
    ...
    if new_data['time'] + travel_time >= 4:
        new_data['day'] += int((new_data['time'] + travel_time)/4)
        new_data['time'] = (new_data['time'] + travel_time) - int((new_data['time'] + travel_time)/4)*4 # CAMBIAR POR MEDIDA CORRESPONDIENTE 4 es un ejemplo para simplificar los calculos, pero en si seria 288 (pasos de 5 mins en un día)
    else:
...
def data_gather(df_buildings_info, agent_name):
    row = df_buildings_info.loc[df_buildings_info['agent_name'] == agent_name].iloc[-1]
    osmid_data = row['osmid']
    link_to = row['link_to']
    day_data = row['day']
    time_data = row['time']
...
#   Definition of main variables
    ...
    population_EV = int(population_citizens*0.3) # amount of EV equals 30% of the population of the region
    directory = 'C:/Users/asier.divasson/Downloads'
    type_services = ['Living', 'Working', 'Commerce', 'Healthcare', 'Education', 'Entertainment']
#   Initialization of stuff    
    moving_agents = pd.DataFrame(columns=['osmid', 'link_to', 'day', 'time', 'agent_name', 'in_out', 'agent_type', 'archetype', 'SoC'])
    citizens_names = [f'citizen_{i}' for i in range(population_citizens)]
    EV_names = [f'elecvehi_{i}' for i in range(population_EV)]
    df_buildings_info, buildings_df, df_services_buildings = initialization(area, population_citizens, directory, type_services, citizens_names, EV_names)
    time = 1 # Due to init has been made in time 0 day 0 (01/01/20XX, 00.00)
    day = 0
#   Simulation loop
    while day < 2:
        print('#'*20, 'day:', day, '#'*20)
        while time < 4:
            ...
            for agent in moving_agents['agent_name']:
                ...
                if time_data == time and day_data == day:
                    df_buildings_info = pd.concat([df_buildings_info, data.to_frame().T], ignore_index=True)
                    ...
                    print(moving_agents)
            for agent in citizens_names:
                if agent not in moving_agents['agent_name'].values:
                    new_data_agent = data_gather(df_buildings_info, agent)
                    decision, intention = behaviour_module(new_data_agent)
    #               print(decision, intention)
                    if decision == 'move':
                        new_data_agent['day'] = day
                        new_data_agent['time'] = time
                        new_data_agent['in_out'] = 'out'
                        df_buildings_info = pd.concat([df_buildings_info, new_data_agent], ignore_index=True)
                        new_data = next_move(df_services_buildings, new_data_agent.iloc[0], buildings_df, intention)
