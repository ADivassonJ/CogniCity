...
def add_bus_relation(df_buildings, area, directory):
    ...
    ####### ESTOS PLOTS HAY QUE MODIFICAR PARA QUE ME LOS GENERE COMO HTML O ASIN
    # Obtener el mapa de la zona
    #G = ox.graph_from_place(area, network_type='all')
    #fig, ax = plt.subplots(figsize=(10, 10))
    #ox.plot_graph(G, ax=ax, node_size=0, edge_linewidth=0.8, edge_color='grey', show=False, close=False)
    
    # Dibujar el diagrama de Voronoi encima del mapa
    #voronoi_plot_2d(vor, ax=ax,show_points=False, show_vertices=False, line_colors='olive', line_width=1)
    
    # Se vuelve a sacar porque en laversion original se han modificado los puntos para que calculen las cosas
...
def initialize_population(df_buildings, population, EV_percentage):
    ...
    df_actions = pd.DataFrame(columns=['osmid', 'service', 'day', 'time', 'agent_name', 'action', 'agent_type', 'archetype'])
    ...
    for citizen in tqdm(citizen_names, desc="       citizens: ", bar_format="{desc}{percentage:3.0f}%"):
        ...
        if osmid == living_on:
            service_building = 'Living'
        else:
            service_building = 'Working'
        bus = df_buildings.loc[df_buildings['osmid'] == osmid, 'bus'].iloc[0]
        new_row = {'osmid': osmid, 'service':service_building, 'day': day, 'time': time, 'agent_name': citizen, 'action': 'in', 'agent_type': 'citizen', 'archetype': archetype, 'bus': bus}
    ...
    for vehicle in tqdm(vehicles_names, desc="       vehicles: ", bar_format="{desc}{percentage:3.0f}%"):
        ...
        bus = df_buildings.loc[df_buildings['osmid'] == osmid, 'bus'].iloc[0]
        new_row = {'osmid': osmid, 'service':'NaN', 'day': day, 'time': time, 'agent_name': vehicle, 'action': 'in', 'agent_type': 'vehicle', 'archetype': archetype, 'bus': bus}
        df_actions = pd.concat([df_actions, pd.DataFrame([new_row])], ignore_index=True)
...
def behaviour_core(link_to, day, time, archetype_data, service_data):
    ...
    ruta_archivo = os.path.join(directorio_base, archetype_data + ".csv")
    
    # Cargar el archivo CSV
    df_behaviour = pd.read_csv(ruta_archivo)
    df_fila_XX = df_behaviour[df_behaviour['Time'] == hour]
    
    # Verificar si df_fila_XX no está vacío
    if df_fila_XX.empty:
        print(f"No se encontró la hora {hour} en el DataFrame.")
        print(f"Recuerda que 6:00 y 06:00 no se detectan igual...")
        return None, None  # O maneja el error de otra manera

    # Concatenar la primera fila (nombres de las columnas) con la fila de la hora XX
    df_resultado = pd.concat([df_behaviour.head(1), df_fila_XX])

    # Extraer etiquetas y valores
    etiquetas = df_resultado.columns[1:]
    valores = df_resultado.iloc[1, 1:].values
