...
def add_bus_relation(df_buildings):
    ...
    coord_buses = [[0, 0], [1, 4], [2, 1], [3, 3]]
    coordenadas = np.array(coord_buses)
...
def next_move(df_buildings, df_citizens,  data, intention):
    ...
    if intention in ['Living', 'Working']:
        ...
        if new_building == building_name:
            return None, 1        
    for building in df_buildings.iterrows():
        building_dest = df_buildings.loc[df_buildings['osmid'] == building[1]['osmid'], 'coord'].iloc[0]
        distance = great_circle(building_coord, building_dest).meters
        max_distance = 100000 # HA OBTENER EN BASE AL ARQUETIPO
        if intention in building[1]['services'] and distance < max_distance:
            available_options.append(building[1]['osmid'])
    # Take out current position from available destinations
    if building_name in available_options:
        available_options.remove(building_name)
    if available_options:
        ...
        if new_data.loc[0, 'time'] + travel_time >= 4:
            ...
            new_data.loc[0, 'time'] = (new_data.loc[0, 'time'] + travel_time) % steps_captured  # Update time
        else:
