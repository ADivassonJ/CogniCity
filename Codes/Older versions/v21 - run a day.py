...
def add_bus_relation(df_buildings, area, directory):
    ...
    ox.plot_graph(G, ax=ax, node_size=0, edge_linewidth=0.8, show=False, close=False)
    
    # Dibujar el diagrama de Voronoi encima del mapa
    voronoi_plot_2d(vor, ax=ax, show_vertices=False, line_colors='grey', line_width=1)
...
#   Simulation loop
    ...
    for df in range(len(df_names)):
        writer = pd.ExcelWriter(directory+'/'+df_names[df]+'.xlsx')
        df_group[df].to_excel(writer, sheet_name=df_names[df], index=False)
        writer.save()
